...
import { getSessionDetail } from "@/lib/api-client/history";
...
function SessionDetailContent() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const data = await getSessionDetail(profileId, sessionId);
      setSession(data);
      setLoading(false);
    }
    loadData();
  ...
  if (loading) return <div className="p-6 text-center text-gym-muted">Caricamento sessione...</div>;
  if (!session) return <p className="text-red-200">Sessione non trovata.</p>;
